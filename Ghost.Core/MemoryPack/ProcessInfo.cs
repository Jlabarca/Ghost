...
public partial class ProcessInfo : IAsyncDisposable
{
    ...
    [MemoryPackIgnore]
    private ProcessStartInfo _startInfo = default(ProcessStartInfo)!; // Reconstructed OnDeserialized
    ...
    public ProcessMetadata Metadata { get; set; } = default(ProcessMetadata)!; // Ensure set before serializing or handle null
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        try
        {
            if (_isDisposed)
            {
                return; // Double-check after acquiring lock (if acquired)
            }
            _isDisposed = true;
            ...
            if (_process != null)
            {
                ...
                { /* G.LogError("Error detaching events or killing process during disposal: {0}", ex, Id); */
                }
                finally
                {
    ...
    private void ConfigureStartInfo(Dictionary<string, string>? environment = null)
    {
        ...
        {
            foreach ((string? key, string? value) in Metadata.Environment)
            {
    ...
    private async void HandleProcessExited(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                if (_process != null && !_process.HasExited)
                { /* Should not happen if Exited fired */
                }
                if (_process != null)
                {
            ...
            { /* G.LogError("Error getting exit code for {0}: {1}", Id, ex.Message); */
            }
    ...
    public async Task WaitForExitAsync()
    {
        ...
        if (p == null || _isDisposed)
        {
            ...
            if (Status == ProcessStatus.Stopped || Status == ProcessStatus.Failed || Status == ProcessStatus.Crashed)
            {
                return;
            }
