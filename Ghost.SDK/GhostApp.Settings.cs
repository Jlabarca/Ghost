...
#region Configuration Loading and Settings Application
    ...
    private GhostConfig CreateDefaultConfig()
    {
        ...
        string? appNameAttr = appType.Name;
        string? entryAssemblyName = Assembly.GetEntryAssembly()?.GetName().Name?.ToLowerInvariant()
    ...
    private void ApplySettings()
    {
        ...
        GhostAppAttribute? attribute = GetType().GetCustomAttribute<GhostAppAttribute>();
        if (attribute != null)
        {
        ...
        if (Config?.Core?.Settings != null)
        {
            ...
            if (tickSeconds > 0)
            {
                TickInterval = TimeSpan.FromSeconds(tickSeconds);
            }
        }
    ...
    {
        if (Config?.Core?.Settings?.TryGetValue(name, out string? valueStr) == true)
        {
    ...
    {
        return Config?.Core?.Settings?.TryGetValue(name, out string? valueStr) == true ? valueStr : defaultValue;
    }
    ...
    private ConnectionConfiguration GetConnectionConfiguration(GhostConfig config)
    {
        ...
        if (string.IsNullOrEmpty(redisConnStr) && config?.Core?.Settings?.TryGetValue("RedisConnection", out string? confRedisConnStr) == true)
        {
