...
{
    private readonly ConcurrentDictionary<string, List<Action<string, string>>> _callbacks = new ConcurrentDictionary<string, List<Action<string, string>>>();
    ...
    private readonly Dictionary<MessagePriority, TimeSpan> _defaultExpiries = new Dictionary<MessagePriority, TimeSpan>
    {
    ...
    private readonly object _lastTopicLock = new object();
    ...
    public RedisGhostBus(string connectionString)
    {
        ...
        _circuitBreaker = new CircuitBreaker(
                ...
                TimeSpan.FromSeconds(15) // Try to recover after 15 seconds
        );
    ...
    /// </summary>
    public async IAsyncEnumerable<T> SubscribeAsync<T>(string channelPattern, [EnumeratorCancellation] CancellationToken cancellationToken = default(CancellationToken))
    {
        if (_isDisposed)
        {
            throw new ObjectDisposedException(nameof(RedisGhostBus));
        }
        if (string.IsNullOrWhiteSpace(channelPattern))
        {
            throw new ArgumentException("Channel pattern cannot be empty", nameof(channelPattern));
        }
        ...
        else
        {
            ...
            try
            {
                ...
                foreach (string matchingChannel in matchingChannels)
                {
        ...
        try
        {
            ...
            while (!linkedSource.Token.IsCancellationRequested)
            {
                ...
                {
                    while (channel.Reader.TryRead(out (string, string) item))
                    {
                        (string topic, string messageId) = item;
                        ...
                        {
                            PersistedMessage persistedMessage = await _persistentStorage.GetMessageAsync(messageId);
    ...
    public async Task UnsubscribeAsync(string channelPattern)
    {
        ...
        try
        {
            ...
            {
                if (_subscriptions.TryRemove(id, out CancellationTokenSource? cts))
                {
                    cts.Cancel();
                    cts.Dispose();
                }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        try
        {
            ...
            // Cancel all subscriptions
            foreach ((string _, CancellationTokenSource? cts) in _subscriptions)
            {
                cts.Cancel();
                cts.Dispose();
            }
    ...
    private async Task CheckConnectionAsync()
    {
        try
        {
            ...
            TimeSpan pingResult = await _db.PingAsync();
    ...
    public async Task PublishWithPriorityAsync<T>(string channel, T message, MessagePriority priority, TimeSpan? expiry = null)
    {
        ...
        expiry ??= _defaultExpiries.TryGetValue(priority, out TimeSpan defaultExpiry) ? defaultExpiry : TimeSpan.FromHours(1);
        ...
        try
        {
            ...
            if (_circuitBreaker.IsAllowed)
            {
                try
                {
                    ...
                    transaction.HashSetAsync($"message:{channel}:{messageId}:meta", new[]
                    {
                            new HashEntry("priority", (int)priority), new HashEntry("timestamp", metadata.Timestamp.Ticks),
                            new HashEntry("type", metadata.TypeName)
                    });
    ...
    {
        if (_isDisposed || !_isAvailable || _persistentStorage == null)
        {
            return;
        }
        ...
        try
        {
            ...
            foreach (PersistedMessage? message in messages)
            {
                try
                {
                    // Use Redis transaction for atomic operations
                    ...
                    TimeSpan expiry = message.Metadata.Expiry - DateTime.UtcNow;
                    ...
                    // Add metadata
                    transaction.HashSetAsync($"message:{message.Channel}:{message.Id}:meta", new[]
                    {
                            new HashEntry("priority", (int)message.Metadata.Priority), new HashEntry("timestamp", message.Metadata.Timestamp.Ticks),
                            new HashEntry("type", message.Metadata.TypeName), new HashEntry("persisted", true)
                    });
        ...
        try
        {
            if (_circuitBreaker.IsAllowed)
            {
                try
                {
                    ...
                    foreach (string? channel in matchingChannels)
                    {
                        ...
                        RedisValue lastMessageId = await _db.StringGetAsync($"channel:{channel}:last");
            ...
            if (_persistentStorage != null)
            {
                try
                {
                    ...
                    foreach (PersistedMessage? msg in persistedMessages)
                    {
                        cancellationToken.ThrowIfCancellationRequested();
    ...
    private void NotifySubscribers(string channel, string messageId)
    {
        ...
        foreach ((string? pattern, var callbacks) in _callbacks)
        {
    ...
    private async void CleanupCallback(object state)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                foreach (string? id in completedSubscriptions)
                {
                    if (_subscriptions.TryRemove(id, out CancellationTokenSource? cts))
                    {
                        cts.Dispose();
