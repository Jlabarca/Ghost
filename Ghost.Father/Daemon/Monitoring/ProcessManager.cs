...
public class ProcessManager : IAsyncDisposable
{
    ...
    public async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            foreach (ProcessInfo? state in states)
            {
    ...
    private async Task InitializeSchemaAsync()
    {
        try
        {
            ...
            bool schemaVersionTableExists = await _data.TableExistsAsync("ghost_schema_version");
            ...
            int currentVersion = await _data.QuerySingleAsync<int>(@"
    ...
    private async Task SubscribeToSystemEventsAsync()
    {
        try
        {
            await foreach (SystemEvent? evt in _bus.SubscribeAsync<SystemEvent>("ghost:events"))
            {
                try
                {
    ...
    public async Task<ProcessInfo> RegisterProcessAsync(ProcessRegistration registration)
    {
        ...
        try
        {
            ...
            ProcessMetadata? metadata = new ProcessMetadata(
                    Path.GetFileNameWithoutExtension(registration.ExecutablePath),
                    registration.Type ?? "generic",
            ...
            ProcessInfo? process = new ProcessInfo(
                    ...
                    registration.Arguments ?? string.Empty,
                    registration.WorkingDirectory ?? Path.GetDirectoryName(registration.ExecutablePath),
                    registration.Environment ?? new Dictionary<string, string>()
            );
    ...
    public async Task StartProcessAsync(string id)
    {
        ...
        try
        {
            if (!_processes.TryGetValue(id, out ProcessInfo? process))
            {
            ...
            ProcessConfig? config = _config.GetModuleConfig<ProcessConfig>($"processes:{id}");
            ...
            int attempts = 0;
            Exception? lastError = default(Exception);
    ...
    private async Task HandleProcessRegistrationAsync(SystemEvent evt)
    {
        try
        {
            ...
            ProcessRegistration? registration = MemoryPackSerializer.Deserialize<ProcessRegistration>(evt.Data);
            if (registration == null)
            {
    ...
    {
        if (_processes.TryGetValue(processId, out ProcessInfo? process))
        {
    ...
    private async Task HandleProcessCrashAsync(string processId)
    {
        if (_processes.TryGetValue(processId, out ProcessInfo? process))
        {
            ...
            ProcessConfig? config = _config.GetModuleConfig<ProcessConfig>($"processes:{processId}");
    ...
    public async Task StopAllAsync()
    {
        ...
        foreach (ProcessInfo? process in runningProcesses)
        {
            await process.StopAsync(_shutdownTimeout);
            await _stateManager.SaveProcessAsync(process);
        }
    ...
    public async Task<object> RunCommandAsync(string command, string args, string workingDirectory, bool waitForExit)
    {
        ...
        try
        {
            ProcessInfo? process = new ProcessInfo(
                    ...
                    new ProcessMetadata("Command", "Command", "1.0.0", new Dictionary<string, string>(), new Dictionary<string, string>()),
                    command,
                    args,
                    workingDirectory,
    ...
    public async Task TerminateGhostProcessesAsync(bool force)
    {
        try
        {
            ...
            if (ghostProcesses.Count > 0)
            {
                ...
                {
                    bool confirmKill =
                            AnsiConsole.Confirm("[yellow]Would you like to terminate these processes?[/]");
                    if (!confirmKill)
                    {
                        throw new GhostException(
    ...
    public async Task DiscoverGhostAppsAsync()
    {
        ...
        string? ghostAppPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Ghost", "Apps");

        if (Directory.Exists(ghostAppPath))
        {
            foreach (string? appDir in Directory.GetDirectories(ghostAppPath))
            {
                string? appName = Path.GetFileName(appDir);
                string? appExe = Path.Combine(appDir, $"{appName}.exe");
        ...
        foreach (ProcessRegistration? app in ghostApps)
        {
