...
public static class GhostConfigExtensions
{
    ...
    public static string? GetRedisConnectionString(this GhostConfig config)
    {
        ...
        string? envConnection = Environment.GetEnvironmentVariable("GHOST_REDIS_CONNECTION");
    ...
    public static string? GetPostgresConnectionString(this GhostConfig config)
    {
        ...
        string? envConnection = Environment.GetEnvironmentVariable("GHOST_POSTGRES_CONNECTION");
        if (!string.IsNullOrEmpty(envConnection))
        {
            return envConnection;
        }

        config.Core.Settings.TryGetValue("PostgresConnection", out string? settingsConnection);
        return settingsConnection;
    }
    ...
    {
        return config.Core.UseInMemoryDatabase.GetValueOrDefault() ||
