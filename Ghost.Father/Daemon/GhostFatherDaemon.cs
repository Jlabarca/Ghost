...
#region GhostApp Overrides
    ...
    public override async Task RunAsync(IEnumerable<string> args)
    {
        ...
        TimeSpan interval = Config.Core.MetricsInterval ?? TimeSpan.FromMinutes(1);
        _daemonMetricsReportingTimer = new Timer(ReportDaemonMetricsCallback, null, TimeSpan.FromSeconds(10), interval);
...
#region Daemon Specific Initialization and Operations
    ...
    private async Task RegisterSelfDirectlyAsync()
    {
        ...
        try
        {
            ...
            AppConnectionInfo? daemonConnectionInfo = new AppConnectionInfo // Defined in GhostFatherDaemon.cs or accessible namespace
            {
                    Id = _daemonId,
...
#region Command Handlers (Example: Ping)
    ...
    private async Task HandlePingCommandAsync(SystemCommand cmd)
    {
        try
        {
            string? responseChannel = cmd.Parameters.GetValueOrDefault("responseChannel", "ghost:responses:unknown");
            G.LogInfo($"Received ping command: {cmd.CommandId}. Will respond on: {responseChannel}");

            Process? daemonProcess = Process.GetCurrentProcess();
    ...
    private async Task HandleStatusCommandAsync(SystemCommand cmd)
    {
        ...
        try
        {
            ...
            {
                ProcessInfo? process = await _processManager.GetProcessAsync(processId);
...
#region Metrics Reporting for Daemon Itself
    ...
    private void ReportDaemonMetricsCallback(object state)
    {
        ...
        try
        {
            ProcessMetrics? daemonMetrics = GetDaemonInternalMetrics();
    ...
    private ProcessMetrics GetDaemonInternalMetrics()
    {
        ...
        return new ProcessMetrics(
                _daemonId,
                0, // Calculating CPU requires more complex logic (time-based sampling)
