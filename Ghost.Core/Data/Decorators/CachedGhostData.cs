...
#region IAsyncDisposable
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        // Dispose all locks
        foreach (SemaphoreSlim? lockObj in _locks.Values)
        {
            lockObj.Dispose();
        }
...
#region Key-Value Operations
    ...
    public async Task<T?> GetAsync<T>(string key, CancellationToken ct = default(CancellationToken))
    {
        ...
        if (_config.Value.UseL1Cache.GetValueOrDefault())
        {
        ...
        SemaphoreSlim keyLock = GetKeyLock(key);
        ...
        try
        {
            ...
            T? fetchedValue = await _inner.GetAsync<T>(key, ct);

            // Store in L1 cache if enabled
            if (fetchedValue != null && _config.Value.UseL1Cache.GetValueOrDefault())
            {
...
#region Batch Key-Value Operations
    ...
    public async Task<IDictionary<string, T?>> GetBatchAsync<T>(IEnumerable<string> keys, CancellationToken ct = default(CancellationToken))
    {
        ...
        if (_config.Value.UseL1Cache.GetValueOrDefault())
        {
            ...
            {
                string cacheKey = GetCacheKey(key, typeof(T));
                T? cachedValue = await _memoryCache.GetAsync<T>(cacheKey, ct);
        ...
        foreach ((string key, T? value) in innerResults)
        {
            result[key] = value;

            if (value != null && _config.Value.UseL1Cache.GetValueOrDefault())
            {
                await _memoryCache.SetAsync(
                        GetCacheKey(key, typeof(T)),
                        value,
