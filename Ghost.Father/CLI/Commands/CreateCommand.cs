...
public class CreateCommand : AsyncCommand<CreateCommand.Settings>
{
    ...
    public CreateCommand()
    {
        try
        {
            ...
            foreach (TemplateInfo? template in templates.Values)
            {
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
    {
        try
        {
            ...
            bool hasLocalLibs = false;

            if (!string.IsNullOrEmpty(ghostInstallDir))
            {
                string? libsPath = Path.Combine(ghostInstallDir, "libs");
                if (Directory.Exists(libsPath))
                {
                    string? sdkFile = Path.Combine(libsPath, "Ghost.SDK.dll");
                    string? coreFile = Path.Combine(libsPath, "Ghost.Core.dll");
            ...
            DirectoryInfo? projectDir = await _templateManager.CreateFromTemplateAsync(
...
";
    ...
    {
        [CommandArgument(0, "[name]"), Description("Name of the new Ghost app")]
        public string Name { get; set; }

        [CommandOption("--template"), Description("Template to use (default: full)")]
        public string Template { get; set; } = "full";

        [CommandOption("--description"), Description("Description of the app")]
        public string Description { get; set; }

        [CommandOption("--namespace"), Description("Root namespace for the app")]
        public string Namespace { get; set; }

        [CommandOption("--use-nuget"), Description("Use NuGet packages instead of local references")]
        public bool UseNuget { get; set; }

        [CommandOption("--sdk-version"), Description("SDK version to use")]
