...
public class MonitorCommand : AsyncCommand<MonitorCommand.Settings>
{
    ...
    private readonly ConcurrentDictionary<string, ProcessMetrics> _latestMetrics = new ConcurrentDictionary<string, ProcessMetrics>();
    private readonly object _lockObject = new object();
    private readonly List<LogEntry> _logs = new List<LogEntry>();
    private readonly Dictionary<string, GhostProcessInfo> _processes = new Dictionary<string, GhostProcessInfo>();
    private readonly List<GhostProcessInfo> _processHistory = new List<GhostProcessInfo>();
    private bool _daemonAlive;
    private bool _isInitializing = true;
    private DateTime _lastUpdate = DateTime.UtcNow;

    private bool _monitoring = true;
    private int _onlineProcesses;
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
    {
        try
        {
            ...
            Layout? dashboard = CreateDashboard(settings);
                    ...
                    .StartAsync(async ctx =>
                    {
                        ...
                        while (_monitoring && !cts.Token.IsCancellationRequested)
                        {
                            try
                            {
                                ...
                                Layout? updatedDashboard = CreateDashboard(settings);
    ...
    private async Task SendDiscoveryCommandsAsync()
    {
        try
        {
            ...
            SystemCommand? discoverCommand = new SystemCommand
            {
                    CommandId = Guid.NewGuid().ToString(),
            ...
            SystemCommand? listCommand = new SystemCommand
            {
                    CommandId = Guid.NewGuid().ToString(),
                    CommandType = "list",
                    Parameters = new Dictionary<string, string>()
    ...
    {
        TimeSpan uptime = DateTime.UtcNow - Process.GetCurrentProcess().StartTime;

        // Show initialization status
        if (_isInitializing)
        {
            Markup? content1 = new Markup("[yellow]Initializing Ghost Monitor...[/]").Centered();
        ...
        string? daemonStatus = _daemonAlive ? "[green]Running[/]" : "[red]Offline[/]";
    ...
    private Table RenderActiveProcessTable(Settings settings)
    {
        ...
        lock (_lockObject)
        {
            ...
            foreach (GhostProcessInfo? process in activeProcesses)
            {
                string? statusColor = process.Status?.ToLowerInvariant() switch
                {
                        "online" => "green",
                        "stopped" => "red",
                        "errored" => "red",
                ...
                string? processType = process.Mode?.ToLowerInvariant() switch
                {
                ...
                string? cpuText = process.CpuUsage.HasValue ? $"{process.CpuUsage:F1}%" : "-";
                string? memoryText = process.MemoryUsage.HasValue ? FormatBytes(process.MemoryUsage.Value) : "-";
                string? uptimeText = process.StartTime.HasValue
    ...
    private Table RenderHistoryTable(Settings settings)
    {
        ...
        lock (_lockObject)
        {
            ...
            foreach (GhostProcessInfo? process in historyProcesses)
            {
                string? statusColor = process.Status?.ToLowerInvariant() switch
                {
                        "stopped" => "blue",
                ...
                string? startText = process.StartTime?.ToString("HH:mm:ss") ?? "-";
                string? endText = process.EndTime?.ToString("HH:mm:ss") ?? "-";
                string? durationText = process.StartTime.HasValue && process.EndTime.HasValue
    ...
    private Panel RenderProcessDetails()
    {
        ...
        lock (_lockObject)
        {
            ...
            {
                selectedProcess = _processes.TryGetValue(_selectedProcessId, out GhostProcessInfo? process)
                        ? process
            ...
            if (selectedProcess != null)
            {
                ...
                string? statusColor = selectedProcess.Status?.ToLowerInvariant() switch
                {
                        "online" => "green",
                        "stopped" => "blue",
                        "errored" => "red",
                ...
                if (selectedProcess.EndTime.HasValue)
                {
                    ...
                    if (selectedProcess.StartTime.HasValue)
                    {
                        TimeSpan duration = selectedProcess.EndTime.Value - selectedProcess.StartTime.Value;
                        grid.AddRow("[bold]Runtime:[/]", FormatDuration(duration));
                    }
                }
                else if (selectedProcess.StartTime.HasValue)
                {
                    TimeSpan uptime = DateTime.UtcNow - selectedProcess.StartTime.Value;
    ...
    private Panel RenderLogs(Settings settings)
    {
        ...
        lock (_lockObject)
        {
            ...
            foreach (LogEntry? log in filteredLogs)
            {
                ...
                string? sourceName = GetShortName(log.Source);

                string? levelColor = log.Level?.ToLowerInvariant() switch
                {
                        "error" => "red",
                ...
                string? levelText = log.Level?.ToUpper();
    ...
    private async Task<T> WaitForResponseAsync<T>(string channel, CancellationToken ct)
    {
        try
        {
            await foreach (T response in _bus.SubscribeAsync<T>(channel, ct))
            {
                return response;
            }
        }
    ...
    private async Task MonitorProcessesAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            await foreach (object? message in _bus.SubscribeAsync<object>("ghost:events:*", cancellationToken))
            {
                if (!_monitoring)
                {
                    break;
    ...
    private async Task MonitorMetricsAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            await foreach (object? metricsData in _bus.SubscribeAsync<object>("ghost:metrics:*", cancellationToken))
            {
                if (!_monitoring)
                {
                    break;
                ...
                try
                {
                    ...
                    string? processId = ExtractProcessIdFromTopic(topic);

                    AddLog("monitor", "debug", $"Received metrics for process: {processId}");

                    if (!string.IsNullOrEmpty(processId))
                    {
                        ProcessMetrics? metrics = await ExtractMetricsAsync(metricsData, processId);
                        if (metrics != null)
                        {
    ...
    private async Task MonitorLogsAsync(CancellationToken cancellationToken)
    {
        try
        {
            await foreach (object? logData in _bus.SubscribeAsync<object>("ghost:logs:*", cancellationToken))
            {
                if (!_monitoring)
                {
                    break;
    ...
    private async Task PollProcessesAsync()
    {
        try
        {
            ...
            var responseTask = Task.Run(async () =>
            {
                try
                {
                    using CancellationTokenSource? cts = new CancellationTokenSource(3000); // 3 second timeout
    ...
    private async Task ProcessEventMessage(object message)
    {
        try
        {
            GhostProcessInfo? processInfo = await ExtractProcessInfoFromEvent(message);
            if (processInfo != null)
            {
    ...
    private async Task<GhostProcessInfo> ExtractProcessInfoFromEvent(object message)
    {
        try
        {
            ...
            if (!eventData.TryGetValue("Id", out JsonElement idElement) ||
            ...
            string name = null;
            if (eventData.TryGetValue("ProcessName", out JsonElement nameElement) &&
            ...
            string status = null;
            if (eventData.TryGetValue("Status", out JsonElement statusElement) &&
                statusElement.ValueKind != JsonValueKind.Undefined)
            {
                status = statusElement.ToString();
            }

            if (string.IsNullOrEmpty(status) && eventData.TryGetValue("EventType", out JsonElement eventTypeElement) &&
                eventTypeElement.ValueKind != JsonValueKind.Undefined)
            {
                string? eventType = eventTypeElement.ToString().ToLowerInvariant();
            ...
            string mode = null;
            if (eventData.TryGetValue("Mode", out JsonElement modeElement) &&
    ...
    private void UpdateProcess(GhostProcessInfo processInfo)
    {
        lock (_lockObject)
        {
            bool existing = _processes.TryGetValue(processInfo.Id, out GhostProcessInfo? current);
            ...
            {
                GhostProcessInfo? historyEntry = new GhostProcessInfo
                {
                        Id = processInfo.Id,
    ...
    private void UpdateProcessWithMetrics(string processId, ProcessMetrics metrics)
    {
        lock (_lockObject)
        {
            if (_processes.TryGetValue(processId, out GhostProcessInfo? process))
            {
            ...
            {
                GhostProcessInfo? newProcess = new GhostProcessInfo
                {
                        Id = processId,
    ...
    private void ProcessLogMessage(object logData)
    {
        try
        {
            LogEntry? logEntry = ExtractLogEntry(logData);
            if (logEntry != null)
            {
        ...
        try
        {
            ...
            try
            {
                ...
                if (logDict != null)
                {
                    ...
                    if (logDict.TryGetValue("Timestamp", out JsonElement tsElement) &&
                        tsElement.ValueKind != JsonValueKind.Undefined &&
                        DateTime.TryParse(tsElement.ToString(), out DateTime parsedTs))
                    {
                        timestamp = parsedTs;
                    }

                    string source = "unknown";
                    if (logDict.TryGetValue("Source", out JsonElement srcElement) &&
                    ...
                    string level = "info";
                    if (logDict.TryGetValue("Level", out JsonElement levelElement) &&
                    ...
                    string message = "";
                    if (logDict.TryGetValue("Message", out JsonElement msgElement) &&
                ...
                try
                {
                    ...
                    if (metricsDict != null)
                    {
                        return new ProcessMetrics(
                                processId,
                                TryGetDouble(metricsDict, "CpuPercentage"),
                                TryGetLong(metricsDict, "MemoryBytes"),
                                TryGetInt(metricsDict, "ThreadCount"),
                                DateTime.UtcNow,
    ...
    private long TryGetLong(Dictionary<string, JsonElement> dict, string key)
    {
        if (dict.TryGetValue(key, out JsonElement element) &&
            (element.ValueKind == JsonValueKind.Number || element.ValueKind == JsonValueKind.String))
        {
            if (long.TryParse(element.ToString(), out long result))
            {
                return result;
            }
        }
    ...
    {
        [CommandOption("--refresh"), Description("Refresh interval in milliseconds"), DefaultValue(500)]
        public int RefreshInterval { get; set; }

        [CommandOption("--lines"), Description("Number of log lines to show"), DefaultValue(12)]
        public int LogLines { get; set; }

        [CommandOption("--history"), Description("Number of historical processes to show"), DefaultValue(5)]
