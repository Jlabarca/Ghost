...
public class RunCommand : AsyncCommand<RunCommand.Settings>
{
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
    {
        try
        {
            ...
            // Register process in database if monitoring is enabled
            if (!settings.DisableMonitoring)
            {
                var processInfo = await _db.RegisterProcess(
                    Path.GetFileNameWithoutExtension(url),
                    Environment.ProcessId,
                    0  // Port will be detected later if applicable
                );

                // Start metrics collection
                var collector = new MetricsCollector(_db, processInfo.Id, _logger);
                await collector.StartAsync();

                AnsiConsole.MarkupLine($"[green]Process monitoring enabled. ID: {processInfo.Id}[/]");
            }
