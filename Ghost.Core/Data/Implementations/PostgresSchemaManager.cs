...
public class PostgresSchemaManager : ISchemaManager
{
    ...
    public async Task InitializeAsync<T>(CancellationToken ct = default(CancellationToken)) where T : class
    {
        ...
        try
        {
            ...
            bool tableExists = await _db.TableExistsAsync(tableName, ct);
            if (!tableExists)
            {
            ...
            else
            {
                ...
                bool isValid = await ValidateTableAsync(tableName, columns, ct);
                if (!isValid)
                {
    ...
    public async Task InitializeAsync(Type[] types, CancellationToken ct = default(CancellationToken))
    {
        ...
        try
        {
            ...
            foreach (Type type in types)
            {
                ...
                MethodInfo method = typeof(PostgresSchemaManager)
    ...
    public async Task MigrateAsync<T>(CancellationToken ct = default(CancellationToken)) where T : class
    {
        ...
        try
        {
            ...
            // Add missing columns
            foreach (ColumnInfo column in columnsToAdd)
            {
...
#region Helper Methods
    ...
    private async Task CreateTableAsync(string tableName, IEnumerable<ColumnInfo> columns, CancellationToken ct)
    {
        ...
        {
            ColumnInfo? column = columnsArray[i];
    ...
    {
        string? addColumnSql = $"ALTER TABLE {tableName} ADD COLUMN {column.Name} {MapToPgType(column.Type)}";
    ...
    private async Task CreateIndexesAsync<T>(string tableName, CancellationToken ct) where T : class
    {
        ...
        string? createIndexSql = $"CREATE INDEX IF NOT EXISTS {indexName} ON {tableName} (id)";
    ...
    private async Task<bool> ValidateTableAsync(string tableName, IEnumerable<ColumnInfo> expectedColumns, CancellationToken ct)
    {
        ...
        foreach (ColumnInfo? expectedColumn in expectedColumns)
        {
            string? columnName = expectedColumn.Name.ToLowerInvariant();

            // Check if column exists
            if (!actualColumnDict.TryGetValue(columnName, out ColumnInfo? actualColumn))
            {
    ...
    private IEnumerable<ColumnInfo> GetColumns<T>() where T : class
    {
        ...
        foreach (PropertyInfo? prop in typeof(T).GetProperties())
        {
            ...
            columns.Add(new ColumnInfo(
                    ...
                    MapCSharpTypeToDbType(prop.PropertyType),
    ...
    private string NormalizeType(string type)
    {
        ...
        string? normalized = Regex.Replace(type, @"\(\d+\)", string.Empty);
