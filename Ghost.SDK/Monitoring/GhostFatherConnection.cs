...
public class GhostFatherConnection : IAsyncDisposable
{
            ...
            IConnectionDiagnostics diagnostics = null)
    {
        ...
        _isDaemonSelf = metadata.Name == "Ghost Father Daemon" ||
                        (metadata.Configuration != null &&
                         metadata.Configuration.TryGetValue("AppType", out var appType) &&
                         appType == "daemon");
    ...
    private async Task RegisterWithDaemonAsync()
    {
        ...
        try
        {
            ...
            if (_usingFallbackComm && _directComm != null)
            {
                ...
                G.LogInfo($"Registered with GhostFather using direct communication: {_metadata.Name} ({_id})");
            }
            else
            {
